fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    let x_values = vec3<i32>(
        (x - 1 + i32(size.x)) % i32(size.x),
        (x + i32(size.x)) % i32(size.x),
        (x + 1 + i32(size.x)) % i32(size.x),
    );
    let y_values = vec3<i32>(
        (y - 1 + i32(size.y)) % i32(size.y),
        (y + i32(size.y)) % i32(size.y),
        (y + 1 + i32(size.y)) % i32(size.y),
    );
    
    sum += textureLoad(input_texture, vec2<i32>(x_values[0],y_values[0]), 0).a * filter_data[0][0];
    sum += textureLoad(input_texture, vec2<i32>(x_values[1],y_values[0]), 0).a * filter_data[0][1];
    sum += textureLoad(input_texture, vec2<i32>(x_values[2],y_values[0]), 0).a * filter_data[0][2];
    ...
    sum += textureLoad(input_texture, vec2<i32>(x_values[0],y_values[2]), 0).a * filter_data[2][0];
    sum += textureLoad(input_texture, vec2<i32>(x_values[1],y_values[2]), 0).a * filter_data[2][1];
    sum += textureLoad(input_texture, vec2<i32>(x_values[2],y_values[2]), 0).a * filter_data[2][2];
